...
def erstelle_buchungs_stempel(daten):
    ...
    betrag_netto     = daten.get("betrag_netto", 0)
    mwst_betrag      = daten.get("mwst_betrag", 0)
    betrag_brutto    = daten.get("betrag_brutto", 0)
    mwst_satz        = daten.get("mwst_satz", 8.1)
    ...
    hoehe = 62*mm if vorsteuer_aktiv else 54*mm
...
def erstelle_bezahlt_stempel(bezahlt_am, bezahlt_von=""):
    ...
    c.setFont("Helvetica-Bold", 9)
    c.drawCentredString(x + breite/2, y + hoehe - 13*mm, bezahlt_am)

    if bezahlt_von:
        c.setFillColor(Color(0.3, 0.3, 0.3, alpha=0.8))
        c.setFont("Helvetica", 5.5)
        c.drawCentredString(x + breite/2, y + 2*mm, bezahlt_von)
...
@app.route("/stempel/kontierung", methods=["POST"])
def stempel_kontierung():
    """Buchungsstempel nach Kontierung freigeben"""
    try:
        # Auth prüfen
        ...
        log(f"🖊️ Buchungsstempel für Dokument: {dokument_id}")
        ...
        # Gestempeltes PDF hochladen
        dateiname = f"kontiert_{dokument_id}.pdf"
        pdf_url = lade_pdf_hoch(pdf_gestempelt, dateiname)

        if not pdf_url:
            return jsonify({"error": "Upload fehlgeschlagen"}), 500

        log(f"✅ Buchungsstempel gesetzt: {dokument_id}")
        return jsonify({"success": True, "pdf_url": pdf_url})

    except Exception as e:
        log(f"❌ Kontierung Stempel Fehler: {e}")
        return jsonify({"error": str(e)}), 500

@app.route("/stempel/bezahlt", methods=["POST"])
def stempel_bezahlt():
    """Bezahlt-Stempel nach Zahlung markieren"""
    try:
        # Auth prüfen
        agent_key = request.headers.get("x-agent-key", "")
        if agent_key != BELEGFLUSS_KEY:
        ...
        log(f"💚 Bezahlt-Stempel für Dokument: {dokument_id}")

        # Aktuelles PDF herunterladen (mit Buchungsstempel)
        pdf_response = requests.get(pdf_url_aktuell, timeout=30)
        if pdf_response.status_code != 200:
            return jsonify({"error": "PDF nicht ladbar"}), 400
        pdf_daten = pdf_response.content

        # Bezahlt-Stempel erstellen
        stempel = erstelle_bezahlt_stempel(bezahlt_am, bezahlt_von)

        # Letzter Seite Bezahlt-Stempel hinzufügen (unten links)
        reader = PdfReader(io.BytesIO(pdf_daten))
        letzte_seite = len(reader.pages) - 1
        pdf_gestempelt = stempel_auf_pdf(pdf_daten, stempel, seite_index=letzte_seite)

        # Gestempeltes PDF hochladen
        dateiname = f"bezahlt_{dokument_id}.pdf"
        neue_pdf_url = lade_pdf_hoch(pdf_gestempelt, dateiname)

        if not neue_pdf_url:
            return jsonify({"error": "Upload fehlgeschlagen"}), 500

        log(f"✅ Bezahlt-Stempel gesetzt: {dokument_id}")
        return jsonify({"success": True, "pdf_url": neue_pdf_url})

    except Exception as e:
        log(f"❌ Bezahlt Stempel Fehler: {e}")
        return jsonify({"error": str(e)}), 500
...
{{
  ...
  "zusammenfassung": "1 Satz auf Deutsch"
    ...
    try:
        ...
        netto, mwst, brutto = berechne_betraege(result.get("betrag_netto", 0), result.get("mwst_satz", 8.1))
...
def verarbeite_mail(mail, mail_id):
    ...
    for pdf in mail_daten["pdfs"]:
        ...
        # Eingangs-Stempel (oben rechts, transparent)
        ...
        ergebnis = speichere_in_belegfluss(analyse, pdf["dateiname"], mail_daten["datum"], pdf_url, pdf_original_url)
        if ergebnis:
...
if __name__ == "__main__":
    ...
    # Mail-Checker in separatem Thread
